...
const International = ({getArticles, article:{articles, loading}}) => {
    useEffect(()=>{
        getArticles();
    },[getArticles])
    return <Fragment>
        {loading ? (<Spinner />) : (<Fragment>
                <div className="col-md-12">
                    <div className="row">
                        {articles.length > 0 ? (
                            articles.filter(article => (article.genre==="International" || article.genre==="international")).map(art=>(
...
export default connect(mapStateToProps,{getArticles})(International);